...
StadiumTree::StadiumTree(StadiumNode* root, char sortCriteria): root(root), sortCriteria(sortCriteria) {
    if (sortCriteria == 's')
        (*compare)(stadium&, stadium&) = compName;
    else if (sortCriteria == 'n')
        (*compare)(stadium&, stadium&) = compTeam;
    else
        (*compare)(stadium&, stadium&) = compDate;
}

//Copy Constructor
StadiumTree::StadiumTree(const StadiumTree& otherTree) {
    root = NULL;
    (*compare)(stadium&, stadium&) = otherTree.compare;
...
***********************************************************/
void StadiumTree::insertNode(stadium val) {
...
***********************************************************/
void StadiumTree::insert(stadiumNode*& nodePtr, stadiumNode*& newNode) {
    //Empty spot reached
    if (nodePtr == NULL)
        nodePtr = newNode;
    //Move to left branch
...
void StadiumTree::showNode(stadiumNode* target) {
    ...
    if (target)
        std::cout << "Results: " << target->value; //Output target's value
    else
    {
...
stadium* StadiumTree::compileInOrder() {
    ...
    int size = size();
    stadium* sortedStadiums = new stadium[size];

    //fill array
    accessInOrder(root, sortedStadiums, 0);
...
void StadiumTree::deleteNode(stadium val, stadiumNode*& nodePtr) {
    ...
    else if (copare(val, nodePtr->value) == 1)
