...
struct graphNode
{
    stadium value; // CALC/OUT - stadium value of the node
                   // CALC/OUT - Distance between this node and a previous one
                   // (ambiguous for potential use
                   // for adjacency list and linked list).
    int distance;
    ...
    ******************************/
    ...
    graphNode(stadium value, int distance = 0, graphNode *adjacent = nullptr)
        : value(value), distance(distance), adjacent(adjacent) {}
...
public:
    ...
    ** CONSTRUCTORS & DESTRUCTOR **
    ******************************/
    stadiumGraph() : adjacencyList(nullptr), vertices(0) {} // Default constr
...
private:
    // CALC/OUT - adjacency list containing all
    // nodes and the nodes they're adjacent to
    graphNode **adjacencyList;
    // CALC/OUT - the number of nodes/vertices
    // in the graph, also size of adjacencyList
    int vertices;
